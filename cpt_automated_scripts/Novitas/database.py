...
class SupabaseHandler:
    """Handle Supabase database operations for Novitas Solutions data with historical archival"""
    ...
    def __init__(self):
        ...
        self.script_name = "Novitas Solutions"
        self.data_types = ["OBL", "Medicare Professional"]  # Multiple data types
    ...
    def insert_records(self, records: List[Dict]) -> dict:
        ...
        try:
            ...
            obl_count = sum(1 for r in existing_records if r.get('data_type') == 'OBL')
            professional_count = sum(1 for r in existing_records if r.get('data_type') == 'Medicare Professional')
            
            logger.info(f"   Found {records_to_archive} existing records total:")
            logger.info(f"      - OBL: {obl_count}")
            logger.info(f"      - Medicare Professional: {professional_count}")
            ...
            new_obl_count = sum(1 for r in records if r.get('data_type') == 'OBL')
            new_professional_count = sum(1 for r in records if r.get('data_type') == 'Medicare Professional')
            
            logger.info(f"   ✅ Successfully inserted {records_to_insert} new records:")
            logger.info(f"      - OBL: {new_obl_count}")
            logger.info(f"      - Medicare Professional: {new_professional_count}")
